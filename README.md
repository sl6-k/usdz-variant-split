# 整机配色变体导出（Blender 4.2.1）

这版 `split_usdz.py` 已替换之前的网格拆分逻辑：将同一手机的黑色、蓝色、红色等配色分别导出，每个 USDZ 都包含完整手机。不会按材质、面或连通块拆开细小网格。
...
```text
/Phone  [color]  当前=red  可选=black, blue, red
```

这里 `/Phone` 是携带变体的节点，`color` 是变体集，`black/blue/red` 是选项。

## 2. 每个配色导出一台完整手机

```bash
python3 split_usdz.py phone.usdz -o result \
  --prim /Phone --variant-set color
```
...
```text
result/
  01_phone_black.usdz     # 完整黑色手机
  02_phone_blue.usdz      # 完整蓝色手机
  03_phone_red.usdz       # 完整红色手机
...
```bash
python3 split_usdz.py phone.usdz -o selected_colors \
  --prim /Phone --variant-set color --variants black blue
```
...
```powershell
py -3 split_usdz.py "D:\models\phone.usdz" -o "D:\models\result" --blender "C:\Program Files\Blender Foundation\Blender 4.2\blender.exe" --prim /Phone --variant-set color
```

Linux：

```bash
python3 split_usdz.py phone.usdz -o result --blender /path/to/blender
```
...
```bash
python3 split_usdz.py phone.usdz --select /Phone:model=Pro --list
```

随后按实际输出中的节点和变体集导出：

```bash
python3 split_usdz.py phone.usdz -o pro_colors \
  --select /Phone:model=Pro --prim /Phone --variant-set color
```
...
```bash
python3 split_usdz.py phone.usdz -o result --runtime blender
```
...
尚未取得你的实际手机文件，因此还没有确认它是否使用 Variant Sets。实际文件可先执行 `--list` 检查。`--runtime blender` 后台启动路径在此环境仍受 Metal 初始化崩溃影响；默认 bundled 路径已实际运行成功。
